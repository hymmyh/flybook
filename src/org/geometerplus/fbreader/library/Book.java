...
public class Book {
	...
	Book(long id, ZLFile file, String title, String encoding, String language, String znflag) {
		myId = id;
		File = file;
		myTitle = title;
		myEncoding = encoding;
	...
	public void setZnFlag(String sflag){
//		System.out.println("---sflag--"+sflag);
		if(sflag==null||sflag.equals("0")){
			znflag=false;
		}else{
			znflag=true;
		}
		...
		database.executeAsATransaction(new Runnable() {
			public void run() {
				if (myId >= 0) {
					...
					database.updateBookInfo(myId, fileInfos.getId(File), myEncoding, myLanguage, myTitle,znflag);
				} else {
					myId = database.insertBookInfo(File, myEncoding, myLanguage, myTitle,znflag);
				}
