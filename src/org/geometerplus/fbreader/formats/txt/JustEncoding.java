...
public class JustEncoding {
    ...
    public static void main(String argc[]) {
        JustEncoding sinodetector;
        int result = OTHER;

        argc = new String[1];
        // argc[0] = "c:\\chinesedata\\codeconvert\\voaunit.txt";
        argc[0] = "中文";
        sinodetector = new JustEncoding();
    ...
    public int detectEncoding(File testfile) {
        ...
        if(size<2){//hym add 如果文件太小 只有一个字节，直接返回。
//        	System.out.println("hym:"+testfile.getPath());
        	return 0;
        }
        try {
    ...
    int hz_probability(byte[] rawtext) {
        ...
        for (i = 0; i+1 < rawtextlen; i++) {//hym 修改
            if (rawtext[i] == '~') {
                if (rawtext[i + 1] == '{') {
                    hzstart++;
                    i += 2;
