...
public class ZLTextPlainModel implements ZLTextModel {
	...
	//hym 保存图片的 段落号，根据段落号能找出前面有多少图片。
	private final ArrayList imglist =new ArrayList();
...
//		myImageMap.put(id, image);
	...
	public int getCPicNum(String id){
		int idnum=Integer.parseInt(id);
		int i=0;
		for(;i<imglist.size();i++){
			int pnum=Integer.parseInt((String)imglist.get(i));
			if(pnum>idnum){
				if(i>0)
					return i-1;
				else
