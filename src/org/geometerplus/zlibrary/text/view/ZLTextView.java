...
public abstract class ZLTextView extends ZLTextViewBase {
	...
	//hym 获得 当前的 pic
	private final synchronized int getCurrentPicNumber(int viewPage, boolean startNotEndOfPage) {
		if ((myModel == null) || (myModel.getParagraphsNumber() == 0)) {
			return 0;
		}
		ZLTextPage page = getPage(viewPage);
		...
		if (startNotEndOfPage) {
			...
			final ZLTextParagraphCursor paragraphCursor = page.EndCursor.getParagraphCursor();
...
//		---
	...
	private synchronized float computeCharsPerPage() {//是平均 每页 显示的 字符数
	...
	private synchronized int computeRealTextPageNumber(int textSize) {
		if (myModel == null || myModel.getParagraphsNumber() == 0) {
			return 1;
		}
		...
		return Math.max((int) (pages + 1.0f - 0.5f * factor), 0);
	}
	private synchronized int computeTextPageNumber(int textSize) {
		if (myModel == null || myModel.getParagraphsNumber() == 0) {
			return 1;
		}
//		System.out.println("----------"+computeCharsPerPage());
		final float factor = 1.0f / computeCharsPerPage();
		final float pages = textSize * factor;
	...
	//获得 整本书的 页码  sizeOfFullText() 获得书中 txt 总数量。不包含图形
	public final synchronized int computePageNumber() {
		//hym  加上图片的 张数。
		int picnum = 0;
		if (myModel != null && myModel.getParagraphsNumber() != 0) {
			picnum=myModel.getPicNum();
		}
		int txtnum=computeRealTextPageNumber(sizeOfFullText());
		int allnum=txtnum+picnum;
//		System.out.println("hym ----txtn"+txtnum+"--picnum"+picnum);
		if(txtnum-picnum>10){//图片太少的
			allnum=txtnum;
		}
		if(allnum<=0){
			allnum=1;
	...
	public final synchronized int computeCurrentPage() {
		...
		int txtnumall=computeRealTextPageNumber(sizeOfFullText());
		int txtnum=computeTextPageNumber(getCurrentCharNumber(PAGE_CENTRAL, false));
		int allnum=txtnum+getCurrentPicNumber(PAGE_CENTRAL, false);
		if(txtnumall-picnum>10){
			allnum=txtnum;
		}
		
		ZLTextPage page = getPage(PAGE_CENTRAL);
		preparePaintInfo(page);
		final ZLTextParagraphCursor paragraphCursor = page.StartCursor.getParagraphCursor();
		if (paragraphCursor == null) {
			return 1;
		}
		//获得 当前段落index。
		final int paragraphIndex = paragraphCursor.Index;
		if(paragraphIndex<=0&&allnum==2){//防止 第1页，页码不为1
			allnum=1;
		}
//		System.out.println("-hym--"+txtnum+"--show num-"+allnum+"--"+paragraphIndex);
		return allnum;
	}
		...
		for (int wordIndex = info.RealStartElementIndex; (wordIndex != endElementIndex) && (index < to); ++wordIndex, charIndex = 0) {
			...
			if (element == area.Element) {
				...
				if (element instanceof ZLTextWord) {
					...
					//把图片 画到现实界面bmp上面。hym 修改，空的x太多 改小为2
//					context.drawImage(areaX, areaY, ((ZLTextImageElement)element).ImageData);
//					System.out.println("-111---hym draw bmp-x--"+areaX);
					context.drawImage(2, areaY, ((ZLTextImageElement)element).ImageData);
	...
	//确定显示 txt 的结束位置。 重要：用来确定 没一页的txt的内容。
	//根据字体的大小 高度 还有屏幕的高度、宽度等来确认可以显示多少
	//TXT文字。 hym
	private void buildInfos(ZLTextPage page, ZLTextWordCursor start, ZLTextWordCursor result) {
//		System.out.println("----hym---create page start");
...
//				System.out.println("----hym---create page info h:"+info.Height+"--textAreaHeight:"+textAreaHeight);
	...
	//显示 一个 page hym  怎么把 page 画到 bmp上面？？？ 见上面  paint
